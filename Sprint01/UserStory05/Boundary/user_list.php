...
<body>
    ...
    <?php if ($view === 'list' || $view === 'search'): ?>
        <?php
        if (!isset($users)) {
            header('Location: ../Controller/ViewUsersController.php');
            exit;
        }
        $heading = ($view === 'search') ? 'Search Results' : 'User List';
        ?>
        <h1><?php echo $heading; ?></h1>
        <form method="get" action="../Controller/SearchUsersController.php" style="margin-bottom:12px;">
            <input type="text" name="q" placeholder="Search by name or email" value="<?php echo htmlspecialchars($searchQ); ?>" required>
            <button type="submit">Search</button>
            <?php if ($view === 'search'): ?>
                <a href="../Controller/ViewUsersController.php" style="margin-left:8px;">Clear</a>
            <?php endif; ?>
        </form>

        <?php if ($view === 'search'): ?>
            <p>Showing results for: <strong><?php echo htmlspecialchars($searchQ); ?></strong></p>
        <?php endif; ?>
                ...
                <tbody>
                    <?php foreach ($users as $u): ?>
                        <tr>
                            ...
                            <td><a href="../Controller/EditUserController.php?id=<?php echo (int)$u->id; ?>">Edit</a></td>
                            <td>
