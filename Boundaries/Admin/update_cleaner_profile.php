...
<div class="container">
    ...
    <form method="POST">
		...
		<label>Rating:</label>
        <input type="text" name="rating" value="<?= htmlspecialchars($profile['rating']) ?>" required>

        <label>Experience:</label>
        <textarea name="experience" rows="3" required><?= htmlspecialchars($profile['experience']) ?></textarea>
		
        <label>Expertise:</label>
        <select name="expertise" required>
            <option value="">Select Service Category</option>
            <?php foreach ($categories as $category): ?>
                <option value="<?= $category['category_id'] ?>" <?= $profile['expertise'] == $category['category_id'] ? 'selected' : '' ?>>
